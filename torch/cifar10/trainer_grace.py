...
from grace_dl.dist.helper import timer, volume
...
parser.add_argument('--grace_config', default='{"compressor": "none", "memory": "none", "communicator": "allreduce"}',
...
def init_wandb(args):
    ...
    if args.rank in [0, -1]:
        ...
        os.environ['WANDB_NAME'] = f"DReduce"
...
def main():
    ...
    # broadcast model states from rank0 to other nodes !!! This is a critical step
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for index, (name, p) in enumerate(model.named_parameters()):
            new_grad = grc.step(p.grad.data, name)
            p.grad.data = new_grad

        optimizer.step()

        # add zero grad if not use DDP
        ...
        if args.log_volume and args.rank in [0,]:
